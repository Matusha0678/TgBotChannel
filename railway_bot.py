...
class RailwayBot:
    ...
    def reset_webhook(self):
        """Сброс webhook для использования polling"""
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get('ok'):
                    print("✅ Webhook сброшен для polling")
                    self.webhook_set = False
                    self.last_update_id = 0
                    return True
            
            print(f"❌ Ошибка сброса webhook: {response.text}")
            return False
    ...
    def get_updates(self):
        ...
        try:
            ...
            if data.get('ok'):
                ...
                if error_code == 409:
                    print("⚠️ Конфликт: webhook активен. Сброс...")
                    self.reset_webhook()
                    return []
                elif error_code == 401:
                    print("❌ Ошибка токена!")
                    ...
                    print(f"❌ Ошибка API {error_code}: {error_desc}")
                    return []
...
"""
    ...
    def run(self):
        ...
        print("🍆 Бот Храма Пенесизма запускается на Railway...")
        ...
        while True:
            try:
                ...
                print(f"Ошибка в основном цикле ({consecutive_errors}/{max_errors}): {e}")
                
                if consecutive_errors >= max_errors:
                    print("❌ Слишком много ошибок, бот останавливается")
                    break
                
                time.sleep(5 * consecutive_errors)  # Увеличиваем задержку при ошибках

if __name__ == "__main__":
    if not BOT_TOKEN:
        print("❌ Ошибка: Не настроен BOT_TOKEN в переменных окружения")
        exit(1)
    elif not GROUP_ID:
        print("❌ Ошибка: Не настроен GROUP_ID в переменных окружения")
        exit(1)
    else:
        print(f"🚀 Запуск на Railway с GROUP_ID: {GROUP_ID}")
        bot = RailwayBot()
        bot.run()
